...
import { OrderAndCartDb } from '../database/database.definition';
import { USER_CART_OPTIONS_SEPARATOR, UserOrder } from './service.definition';
...
export async function fetchUserOrders(userId: string): Promise<UserOrder[]> {
    const userOrdersDb: OrderAndCartDb[] = await fetchUserOrdersDb(userId);
    ...
    orderIds.forEach(id => {
        const filtered: OrderAndCartDb[] = userOrdersDb.filter(uo => id === uo.user_order_id);
        const d1 = filtered[0];
